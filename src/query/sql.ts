...
function conditionSql(cond: Condition): string {
  ...
  switch (property.kind) {
    ...
    case 'date': {
      ...
      switch (cond.op) {
        case 'after':
          return min == null ? `/* ${col}: no value */` : `${col} > ${quote(min)}`
        case 'before':
          return max == null ? `/* ${col}: no value */` : `${col} < ${quote(max)}`
        case 'on':
          return min == null ? `/* ${col}: no value */` : `${col} = ${quote(min)}`
        default: // between — one-sided bounds degrade to >= / <=
          if (min == null && max == null) return `/* ${col}: no bounds */`
          if (min != null && max != null) return `${col} BETWEEN ${quote(min)} AND ${quote(max)}`
          return min != null ? `${col} >= ${quote(min)}` : `${col} <= ${quote(max!)}`
      }
    }
  }
}
